...
# Shorthand: run a command with env prefix
run_with_env = APP_ENV=$(ENV) $(1)
...
# ---------------------------------------------------------------------------
migrate:
	@$(call run_with_env,uv run --python 3.12 alembic upgrade head)

migration:
	@if [ -z "$(MSG)" ]; then \
		echo "Usage: make migration MSG=\"describe your change\""; exit 1; \
	fi
	@$(call run_with_env,uv run --python 3.12 alembic revision --autogenerate -m '$(MSG)')

migrate-downgrade:
	@$(call run_with_env,uv run --python 3.12 alembic downgrade -1)
